...
curatedInteresting = [reki, rampa];
        ...
        structure["Культурные и исторические маршруты"]["Набережная"] = [nabOgrazhdenie, rampa, bezdomniipec, nabIntro, reki, nabBridges, nabfonar, nabteremok, nabFences, nabCinema, nabHeart];
        ...
        structure["Культурные и исторические маршруты"]["Легенды и мифы города"] = [reki, rampa, pristan, olenLesnaya, kubizm, nabOgrazhdenie];
...
curatedRecommended = [erevFountains];
...
const AccountPage = ({ account, onBack, darkMode, setAccount, lang, completedRoutes }) => { 
  const C = darkMode ? S.dark : S.light; 
  const t = (k) => TRANSLATIONS[lang]?.[k] || k; 
...
useEffect(() => {
  ...
  return <AccountPage account={account} onBack={goBack} darkMode={darkMode} setAccount={setAccount} lang={currentLang} completedRoutes={completedRoutes} />;
}
